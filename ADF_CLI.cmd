...
if not exist "!PORTABLE_PYTHON!" (
    echo [ERROR] Portable Python not found after setup. >> "%DEBUG_LOG%"
    pause
    exit /b 1
)

:: ------------------------------------------------------------------
:: DOWNLOAD OR UPDATE PYTHON SCRIPT
:: ------------------------------------------------------------------
call :UPDATE_SCRIPT
if %errorlevel% neq 0 (
...
if !errorlevel! neq 0 (
    ...
    if !errorlevel! neq 0 (
        echo [ERROR] Could not replace script. >> "%DEBUG_LOG%"
...
if "!NEW_VERSION!"=="!REMOTE_VERSION!" (
    ...
    echo [ERROR] Script update verification failed! >> "%DEBUG_LOG%"
    pause
    exit /b 1
)
timeout /t 1 /nobreak >nul
